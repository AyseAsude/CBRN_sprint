...
class ProbeDataset(Dataset):
    ...
    def __init__(self, data: List[Dict[str, Any]], tokenizer, max_length: int = 2048, use_output: bool = True):
        self.data = data
    ...
    def __getitem__(self, idx):
        ...
        if self.use_output:
            # Include both instruction and output
            messages = [
...
def load_combined_dataset(
    benign_path: List[str],
    harmful_path: List[str],
    val_split: float = 0.2,
    random_state: int = 42
) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    """
    Load and combine datasets from multiple JSON files with labels, then perform stratified split.

    Args:
        benign_path: List of paths to benign samples JSON (label=0)
        harmful_path: List of paths to harmful samples JSON (label=1)
    ...
    for path in benign_path:
        logger.info(f"Loading benign samples from {path}")
    ...
    harmful_data = []
    for path in harmful_path:
        logger.info(f"Loading harmful samples from {path}")
        with open(path, 'r') as f:
            data = json.load(f)
...
def create_dataloaders(
    ...
    train_dataset = ProbeDataset(train_data, tokenizer, max_length, use_output=use_output)
    val_dataset = ProbeDataset(val_data, tokenizer, max_length, use_output=use_output)
